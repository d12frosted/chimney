      ...
      case Some(tParam) if config.ignoreNoneInPatch && bothOptions(patchParamTpe, tParam.resultTypeIn(T)) =>
        Some {
          ...
          if (patchParamTpe <:< tParamTpe) {
            ...
            expandTransformerTree(patchField, TransformerConfig(TransformerOptions.default))(
              patchParamTpe,
              tParamTpe
          ...
          expandTransformerTree(patchField, TransformerConfig(TransformerOptions.default))(
            ...
            .flatMap { errors =>
              if (isOption(patchParamTpe)) {
                expandTransformerTree(q"$patchField.get", TransformerConfig(TransformerOptions.default))(
